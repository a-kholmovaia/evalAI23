...
class PlayerHealthbar(DrawableObject):
    def __init__(self, window: Window, scene_path: str, font: pygame.font.Font):
        super().__init__(window, 0, 0, 240, 80)
        self.cur_health = 100
    ...
    def draw(self):
        ...
        self.window.blit(self.font.render("  your  health", True, constants.BLACK), (self.window.get_width() - 13*self.border_padding, self.border_padding  + 5 + self.health_bar_height))

    def scale(self):
        self.x = self.original_x * self.window.get_scale_width()
        self.y = self.original_y * self.window.get_scale_height()
        self.width = self.original_width * self.window.get_scale_width()
        self.height = self.original_height * self.window.get_scale_height()
        self.max_health_width = self.original_max_health_width * self.window.get_scale_width()
        self.health_bar_height = self.original_health_bar_height * self.window.get_scale_height()
        self.border_padding = self.original_border_padding * self.window.get_scale_width()
